...
const Admin = () => {
  ...
  const [showDeleteModal, setShowDeleteModal] =({ show: false, id: null });
  ...
  const fetchNotifications = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/notifications', getAuthHeader());
  ...
  const handleOpenAlerts = async () => {
    ...
    try {
      await axios.put('http://localhost:5000/api/notifications/mark-as-read', {}, getAuthHeader());
      setUnreadCount(0);
  ...
  const confirmClearHistory = async () => {
    try {
      await axios.delete('http://localhost:5000/api/notifications/clear-all', getAuthHeader());
      setNotifications([]);
      setUnreadCount(0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://localhost:5000/api/cars/${editId}`, formData, getAuthHeader());
        setLastId(editId);
      } else {
        const res = await axios.post('http://localhost:5000/api/cars/add', formData, getAuthHeader());
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/cars/${showDeleteModal.id}`, getAuthHeader());
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-['Poppins'] text-slate-900">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-full shadow-sm z-50">
        ...
        <div className="p-6 border-t border-slate-100 space-y-2">
          ...
          <Link to="/" className="w-full flex items-center justify-center gap-2 py-3 bg-slate-50 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all">
            Quitter
          </Link>
        </div>
      </aside>

      {/* CONTENU PRINCIPAL (Pas de Navbar horizontale ici) */}
      <main className="flex-1 ml-64 p-12">
        {activeMenu === 'stats' && (
          <div className="space-y-10 animate-in fade-in duration-500">
            {/* Titre de section ajouté pour compenser l'absence de Navbar */}
            <h2 className="text-[14px] font-bold text-slate-900 uppercase tracking-[0.2em] mb-8 border-l-4 border-[#184f02] pl-4">Tableau de bord</h2>
              ...
              <div className="space-y-4">
                <div className="flex gap-4">
                  {['Tous', 'Voiture', 'Camion', 'Tracteur'].map((cat) => (
                    <button key={cat} onClick={() => setInventoryFilter(cat)} className={`text-[10px] font-bold uppercase tracking-widest px-4 py-2 rounded-lg transition-all ${inventoryFilter === cat ? 'bg-slate-900 text-white shadow-md' : 'text-slate-6