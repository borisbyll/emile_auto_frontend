...
const Admin = () => {
  ...
  const [messages, setMessages] = useState([]); // État pour la messagerie
  ...
  // FETCH MESSAGES (Alerte messagerie)
  const fetchMessages = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/messages`, getAuthHeader());
      setMessages(res.data);
    } catch (err) { console.error("Erreur Messagerie:", err); }
  };
  ...
  const mostViewed = [...vehicles].sort((a, b) => (b.views || 0) - (a.views || 0))[0];
  ...
  const handleEdit = (vehicle) => {
    ...
    setFormData({
      categorie: vehicle.categorie, marque: vehicle.marque, modele: vehicle.modele,
      prix: vehicle.prix, annee: vehicle.annee, valeurCompteur: vehicle.valeurCompteur,
      tonnage: vehicle.tonnage || '', motorisation: vehicle.motorisation,
      transmission: vehicle.transmission, description: vehicle.description, images: vehicle.images
    });
  ...
  const Icons = {
    ...
    Message: () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-['Poppins'] text-slate-900">
        ...
        <nav className="flex-1 px-6 space-y-8 mt-6">
          ...
          <div className="space-y-4">
            {[
              ...
              { id: 'messages', label: 'Messagerie', icon: <Icons.Message />, count: messages.filter(m => !m.read).length }
            ].map((item) => (
              <button key={item.id} onClick={() => setActiveMenu(item.id)} className={`w-full flex items-center justify-between py-3 text-[11px] font-bold uppercase tracking-widest transition-all rounded-xl px-4 ${activeMenu === item.id ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
                <div className="flex items-center gap-4">{item.icon} {item.label}</div>
                {item.count > 0 && <span className="bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full">{item.count}</span>}
              </button>
            ))}
          </div>
        </nav>
      ...
      <main className="flex-1 ml-64 p-12">
          ...
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-3 gap-8">
              <div className="col-span-1 bg-white p-8 rounded-xl border border-slate-100 shadow-sm">
                ...
                {mostViewed ? (
                  <div className="space-y-4">
                    <img src={mostViewed.images[0]} className="w-full h-32 object-cover rounded-lg" alt="Top" />
                ...
                ) : <p className="text-slate-400 text-[11px]">Aucun véhicule</p>}
              </div>
                ...
                <div className="space-y-8">
                  {[ 
                    ...
                    <div key={i} className="space-y-2">
                      <div className="flex justify-between items-end"><span className="text-[11px] font-bold uppercase text-slate-700">{item.label}</span><span className="text-[11px] font-black text-slate-900">{item.stats.count} unités</span></div>
                      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${item.color}`} style={{ width: `${item.stats.percentage}%` }}></div></div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activeMenu === 'publish' && (
          <div className="max-w-4xl animate-in fade-in duration-500">
            <h2 className="text-[12px] font-bold text-slate-900 uppercase mb-10 border-l-4 border-[#184f02] pl-4">{editId ? "Modifier l'actif" : "Ajout de Matériel"}</h2>
            <form onSubmit={handleSubmit} className="bg-white p-10 rounded-xl shadow-sm border border-slate-100 space-y-8">
               <div className="grid grid-cols-4 gap-6">
                ...
                <label className="p-2 border-b border-[#184f02] text-[#184f02] font-bold text-[10px] uppercase cursor-pointer text-center">
            ...
            <div className="flex justify-between items-end border-b border-slate-200 pb-6">
              <div className="flex gap-4">
                {['Tous', 'Voiture', 'Camion', 'Tracteur'].map((cat) => (
                  <button key={cat} onClick={() => setInventoryFilter(cat)} className={`text-[10px] font-bold uppercase px-4 py-2 rounded-lg ${inventoryFilter === cat ? 'bg-slate-900 text-white' : 'text-slate-600'}`}>{cat}</button>
                ))}
              </div>
              <div className="relative w-80">
                <div className="absolute inset-y-0 left-4 flex items-center text-slate-400"><Icons.Search /></div>
                <input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl py-3 pl-12 text-[11px] font-bold outline-none shadow-sm" />
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <table className="w-full text-left">
                <thead className="bg-slate-50 text-[9px] font-bold text-slate-600 uppercase border-b border-slate-100">
                  <tr><th className="p-6">Asset</th><th className="p-6 text-center">Vues</th><th className="p-6 text-right">Actions</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-[12px]">
                  {filteredVehicles.map((v) => (
                    <tr key={v._id} className="hover:bg-slate-50/50">
                      <td className="p-6 flex items-center gap-6">
                        <img src={v.images[0]} className="w-12 h-10 object-cover rounded shadow-sm" alt="" />
                        <div><p className="font-bold text-slate-900 uppercase">{v.marque} {v.modele}</p><p className="text-[10px] text-[#184f02] font-bold">{Number(v.prix).toLocaleString()} €</p></div>
                      </td>
                      ...
                      <td className="p-6 text-right">
                        ...
                        <button onClick={() => handleEdit(v)} className="text-[10px] font-bold uppercase text-[#184f02] ml-4">Modifier</button>
        ...
        {activeMenu === 'messages' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <h2 className="text-[12px] font-bold text-slate-900 uppercase mb-10 border-l-4 border-[#184f02] pl-4">Messages reçus</h2>
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              {messages.length > 0 ? (
                <div className="divide-y divide-slate-100">
                  {messages.map((msg) => (
                    <div key={msg._id} className={`p-6 hover:bg-slate-50 transition-colors ${!msg.read ? 'bg-blue-50/30' : ''}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div><h4 className="font-bold text-slate-900">{msg.nom}</h4><p className="text-[11px] text-slate-500">{msg.email}</p></div>
                        <span className="text-[10px] text-slate-400 font-mono">{new Date(msg.createdAt).toLocaleDateString()}</span>
                      </div>
                      <p className="text-[13px] text-slate-700 leading-relaxed mb-4">{msg.message}</p>
                      <div className="flex gap-4">
                        <a href={`mailto:${msg.email}`} className="text-[10px] font-bold uppercase text-[#184f02] hover:underline">Répondre</a>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="p-20 text-center text-slate-400 text-[13px]">Aucun message pour le moment</div>
              )}
            </div>
          </div>
        )}
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl border border-slate-100">
            <h3 className="text-lg font-bold text-slate-900 mb-6 uppercase tracking-widest">{editId ? "Mis à jour" : "Publication Validée"}</h3>
      ...
      {showDeleteModal.show && (
        <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-8 max-w-sm w-full shadow-2xl border border-slate-100 text-center">
